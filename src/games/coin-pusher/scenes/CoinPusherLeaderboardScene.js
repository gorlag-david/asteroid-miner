...
const LEADERBOARD_KEY = 'coinPusherLeaderboard';
...
export class CoinPusherLeaderboardScene extends Phaser.Scene {
  constructor() {
    super('CoinPusherLeaderboardScene');
  }
  ...
  create() {
    ...
    this.add.text(width / 2, 40, 'TOP PUSHERS', {
    ...
    if (board.length === 0) {
      this.add.text(width / 2, height / 2, 'No scores yet.\nPlay a round to get on the board!', {
        fontSize: '16px', fontFamily: 'monospace', color: '#8888aa', align: 'center',
      }).setOrigin(0.5);
    } else {
      const startY = 100;
      const rowH = 40;
      ...
      this.add.text(width - 120, startY, 'SCORE', {
        fontSize: '14px', fontFamily: 'monospace', color: '#666688',
      }).setOrigin(0.5);
    ...
    const playBtn = this.add.text(width / 2, height - 70, '[ PLAY AGAIN ]', {
      fontSize: '20px', fontFamily: 'monospace', color: '#e94560',
      backgroundColor: '#2a2a4a', padding: { x: 16, y: 6 },
    }).setOrigin(0.5).setInteractive({ useHandCursor: true });

    playBtn.on('pointerover', () => playBtn.setStyle({ color: '#2a2a4a', backgroundColor: '#e94560' }));
    playBtn.on('pointerout', () => playBtn.setStyle({ color: '#e94560', backgroundColor: '#2a2a4a' }));
    playBtn.on('pointerdown', () => this.scene.start('CoinPusherPlayScene'));

    const arcadeBtn = this.add.text(width / 2, height - 30, '[ ARCADE MENU ]', {
    ...
    this.input.keyboard.on('keydown-ENTER', () => this.scene.start('CoinPusherPlayScene'));
    this.input.keyboard.on('keydown-SPACE', () => this.scene.start('CoinPusherPlayScene'));

    if (isMobile) {
      this.input.on('pointerdown', (pointer) => {
        const hitObjects = this.input.hitTestPointer(pointer);
        if (hitObjects.length === 0) this.scene.start('CoinPusherPlayScene');
      });
    }
  }
}
